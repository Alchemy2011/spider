...
def get_robots(url):
    ...
    # 下面这行，在其他地方用的时候需要修改，原来是robots.txt,路径问题改为现状
    # urljoin这个方法比较变态，不同于普通的join，很容易出错。尤其当文件不在根目录下时。
    rp.set_url(urlparse.urljoin(url, '/places/static/robots.txt'))
    rp.read()
    return rp
...
if __name__ == '__main__':
    link_crawler('http://127.0.0.1:8000/places/default', '/places/default/(index|view)', delay=0, num_retries=1,
